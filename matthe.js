...
client.on("ready", () => {
    client.user.setPresence({ activity: { name: "Developed by Matthe" }, status: "online" });
})
client.on("ready", () => {
...
client.on("message", async message => {
    ...
    client.channels.cache.get(config.logkanal).send(new Discord.MessageEmbed().setAuthor("Yeni Bir Öneri Var", client.user.avatarURL()).setFooter("Developed by Matthe").setDescription(`**Gönderenin Bilgileri:** ${message.author} - ${message.author.id}`).setTimestamp().addField("**Mesaj**", message.content).setColor("RANDOM"))
    if (1 > 0) {
...
client.login(config.token).then(() => console.log(`[ MATTHE ] ${client.user.username} olarak giriş yaptı!`)).catch(() => console.log(`[ MATTHE ] Bot giriş yapamadı!`));
